...
class BpdaAdapter():
    def __init__(self, logits_forward, logits_backward, x, y, sess, num_classes=10, device="cpu"):
        self.logits_forward = logits_forward
        self.logits_backward = logits_backward
        self.sess = sess
        self.x_input = x
        self.y_input = y
        ...
        if num_classes <= 10:
            ...
            for cl in range(num_classes):
                # Using logits_backward to approximate the gradient
                self.grads[cl] = tf.gradients(self.logits_backward[:, cl], self.x_input)[0]
        
        # cross-entropy loss
        # Using logits_backward to approximate the gradient
        self.xent = tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits=self.logits_backward, labels=self.y_input)
        self.grad_xent = tf.gradients(self.xent, self.x_input)[0]
        
        # dlr loss
        # Using logits_backward to approximate the gradient
        self.dlr = dlr_loss(self.logits_backward, self.y_input, num_classes=self.num_classes)
        self.grad_dlr = tf.gradients(self.dlr, self.x_input)[0]
        
        # targeted dlr loss
        # Using logits_backward to approximate the gradient
        self.y_target = tf.placeholder(tf.int64, shape=[None])
        self.dlr_target = dlr_loss_targeted(self.logits_backward, self.y_input, self.y_target, num_classes=self.num_classes)
        ...
        la_logit = tf.reduce_sum(la_mask * self.logits_backward, axis=1)
        la_target_logit = tf.reduce_sum(la_target_mask * self.logits_backward, axis=1)
        self.diff_logits = la_target_logit - la_logit
        self.grad_diff_logits = tf.gradients(self.diff_logits, self.x_input)[0]

    def predict(self, x):
            x2 = np.moveaxis(x.cpu().detach().numpy(), 1, 3)
            y = self.logits_forward(x2, self.sess)
    ...
    def grad_logits(self, x):
        ...
        # Use logits_forward to get the actual outputs during the forward pass
        logits = self.logits_forward(x2, self.sess)
        
        g2 = self.sess.run(self.grads, {self.x_input: x2})
        g2 = np.moveaxis(np.array(g2), 0, 1)
        g2 = np.transpose(g2, (0, 1, 4, 2, 3))
        
        return torch.from_numpy(logits).to(self.device), torch.from_numpy(g2).to(self.device)

    def get_grad_diff_logits_target(self, x, y=None, y_target=None):
        la = y.cpu().detach().numpy()
        la_target = y_target.cpu().detach().numpy()
        x2 = np.moveaxis(x.cpu().detach().numpy(), 1, 3)
        
        # Use logits_forward to get the actual outputs during the forward pass
        logits_val = self.logits_forward(x2, self.sess)
        dl = self.sess.run(self.diff_logits, {self.logits_backward: logits_val})
        
        g2 = self.sess.run(self.grad_diff_logits, {self.x_input: x2, self.la: la, self.la_target: la_target})
    ...
    def get_logits_loss_grad_xent(self, x, y):
        x2 = np.moveaxis(x.cpu().detach().numpy(), 1, 3)
        y2 = y.clone().cpu().detach().numpy()
        
        # Use logits_forward to get the actual outputs during the forward pass
        logits_val = self.logits_forward(x2, self.sess)
        
        loss_indiv_val, grad_val = self.sess.run([self.xent, self.grad_xent], {self.x_input: x2, self.y_input: y2})
    ...
    def get_logits_loss_grad_dlr(self, x, y):
        ...
        loss_indiv_val, grad_val = self.sess.run([self.dlr, self.grad_dlr], {self.x_input: x2, self.y_input: y2})
        grad_val = np.moveaxis(grad_val, 3, 1)
    ...
    def get_logits_loss_grad_target(self, x, y, y_target):
        x2 = np.moveaxis(x.cpu().detach().numpy(), 1, 3)
        y2 = y.clone().cpu().detach().numpy()
        y_targ = y_target.clone().cpu().detach().numpy()
        
        # Use logits_forward to get the actual outputs during the forward pass
        logits_val = self.logits_forward(x2, self.sess)
        
        loss_indiv_val, grad_val = self.sess.run([self.dlr_target, self.grad_target], {self.x_input: x2, self.y_input: y2, self.y_target: y_targ})
        grad_val = np.moveaxis(grad_val, 3, 1)
